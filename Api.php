...
class Api
{
    ...
    const BASE_URI = 'https://api.tokkea.exchange.io/';
    const TIMEOUT = 10.0;
     ...
     *
     * No arguments - set TOKKEA_EXCHANGE_API_KEY and TOKKEA_EXCHANGE_API_SECRET in environment
    ...
    {
        $apiKey = getenv('TOKKEA_EXCHANGE_API_KEY');
        $apiSecret = getenv('TOKKEA_EXCHANGE_API_SECRET');
        if ($apiKey && $apiSecret) {
            $this->api_key = $apiKey;
    ...
    {
        $file = is_null($file) ? __DIR__ . "/php-tokkea.exchange-api-config.json" : $file;
